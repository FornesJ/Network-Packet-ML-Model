...
class SplitBenchmark(Benchmark):
    def __init__(self, model, loader, batch_size, model_name, result_path, runs, socket, split="dpu"):
        super().__init__(model, loader, batch_size, model_name, result_path, runs)
        self.socket = socket
        self.split = split
    ...
    def receive(self, data, labels, time=False):
        if self.split == "host":
    ...
    def warmup(self, warmup=10):
        ...
        for i, (data, labels) in enumerate(self.loader):
            data, labels = self.receive(data, labels)
            with torch.no_grad():
    ...
    def transfer_time(self):
        ...
        if self.split == "dpu":
            for data, labels in self.loader:
                data, labels = self.receive(data, labels, time=True)
                with torch.no_grad():
                    features = self.model.model(data)
            ...
            times = []
            for data, labels in self.loader:
                (data, time), labels = self.receive(data, labels, time=True)
                with torch.no_grad():
                    features = self.model.model(data)
                self.send(features, labels, time=True)
                times.append(time)
            ...
            self.results["Transfer"] = {
                "Avg.": f"{sum(times)/len(times)*1000:.3f}",
                "Min": f"{min(times)*1000:.3f}",
                "Max": f"{max(times)*1000:.3f}"
            }
